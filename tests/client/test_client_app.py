...
def make_root_for_happy_path():
    ...
    # Pricing block
    build_nested(root, [57, 0, 0, 0, 0, 1, 0, 0], 1_990_000)  # price micros
    ...
    build_nested(root, [68, 1, 4, 2], "https://play.google.com/store/apps/dev?id=ACME_INC")
    ...
    build_nested(root, [78, 0], [[None, None, None, [None, None, "https://img.test/shot1.png"]]])
...
class ClientAppTest(unittest.TestCase):
    ...
    def _ds5_with_root(self, root):
        # ds:5 structure is such that client uses ds5[1][2]
    ...
    def test_happy_path_extracts_and_transforms(self, mock_get, mock_parse):
        app_id = "com.example.app"
        html = "<html>dummy</html>"
        mock_get.return_value = html

        root = make_root_for_happy_path()
        ds5 = self._ds5_with_root(root)
        mock_parse.return_value = {"ds:5": ds5}

        details = self.client.app(app_id)
        ...
        # Description processing: <br> -> CRLF
        ...
        self.assertFalse(details.free)  # price micros non-zero
        # Availability flags
        ...
        # Note: AppDetails model does not expose `url` field; client computes it
        # but pydantic model ignores extra fields, so we do not assert it here.

        # Ensure network layer was called with expected path and params
    ...
    @patch("google_play_scraper.client.ScriptDataParser.parse")
    @patch("google_play_scraper.client.Requester.get")
    def test_description_fallback_is_used(self, mock_get, mock_parse):
        app_id = "com.example.app"
        mock_get.return_value = "<html>dummy</html>"

        root = []
        # Primary description path missing -> None
        # Provide fallback at [12,0,0,1]
        build_nested(root, [12, 0, 0, 1], "Fallback<br>Desc")
        # Also include minimal required fields to satisfy model
        build_nested(root, [0, 0], "My App")
    ...
    @patch("google_play_scraper.client.ScriptDataParser.parse")
    @patch("google_play_scraper.client.Requester.get")
    def test_missing_ds5_raises_app_not_found(self, mock_get, mock_parse):
        mock_get.return_value = "<html>dummy</html>"
        mock_parse.return_value = {}

        with self.assertRaises(AppNotFound):
            self.client.app("com.missing.app")

    @patch("google_play_scraper.client.ScriptDataParser.parse")
    @patch("google_play_scraper.client.Requester.get")
    def test_malformed_ds5_raises_app_not_found(self, mock_get, mock_parse):
        mock_get.return_value = "<html>dummy</html>"
        # ds5 present but ds5[1][2] will fail
        mock_parse.return_value = {"ds:5": [None, None]}

        with self.assertRaises(AppNotFound):
            self.client.app("com.example.app")
